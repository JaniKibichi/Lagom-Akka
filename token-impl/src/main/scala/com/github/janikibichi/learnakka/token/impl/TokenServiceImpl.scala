package com.github.janikibichi.learnakka.token.impl

import java.util.UUID
import com.github.janikibichi.learnakka.token.api._
  ...
  override def retrieveToken: ServiceCall[RetrieveTokenRequest, RetrieveTokenResult] = ServiceCall {
    request => Future {
      permittedIds.get(request.clientId) match {
        ...
        case Some(secret) if secret == request.clientSecret => val token = UUID.randomUUID().toString
          tokenStore += request.clientId -> token
          RetrieveTokenResult(true, Some(token))

        case _ => RetrieveTokenResult(false)
      }
    }
  }
