...
/**
  * This is an event sourced entity. It has a state, [[LagomscalaState]], which
  ...
  */
class LagomscalaEntity extends PersistentEntity {

  override type Command = LagomscalaCommand[_]
  override type Event = LagomscalaEvent
  override type State = LagomscalaState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: LagomscalaState = LagomscalaState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case LagomscalaState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        LagomscalaState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class LagomscalaState(message: String, timestamp: String)

object LagomscalaState {
  /**
    ...
    */
  implicit val format: Format[LagomscalaState] = Json.format
}

/**
  * This interface defines all the events that the LagomscalaEntity supports.
  */
sealed trait LagomscalaEvent extends AggregateEvent[LagomscalaEvent] {
  def aggregateTag = LagomscalaEvent.Tag
}

object LagomscalaEvent {
  val Tag = AggregateEventTag[LagomscalaEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends LagomscalaEvent
...
/**
  ...
  */
sealed trait LagomscalaCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends LagomscalaCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends LagomscalaCommand[String]
...
/**
  ...
  * application loader.
  */
object LagomscalaSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[LagomscalaState]
  )
}
